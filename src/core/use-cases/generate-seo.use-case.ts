...
export class GenerateSeoUseCase {
  ...
  async execute(product: Product) {
    ...
    // 2. Получение промпта
    const prompt = await this.promptProvider.getPrompt("seo_description_generator", {
    ...
    const observationId = handler.observationId || (handler.getLangchainRunId ? handler.getLangchainRunId() : undefined);

    try {
      const rawContent = (response as any).content?.toString() || response.toString();
